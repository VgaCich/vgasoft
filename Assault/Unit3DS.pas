...
uses Windows, AvL, dglOpenGL, avlVectors;
...
begin
  ...
  FFile.Seek(NewPosition, soFromCurrent);
...
begin
  FBytesReaded:=FFile.Read(FId, 2);
  FBytesReaded:=FBytesReaded + FFile.Read(FLength, 4);
  Result:=FBytesReaded;
end;
...
//  FTransformList.Push;
  ...
  glBegin(FRMode);
  if not Material.HasTexture then glColor4f(Material.Diffuse.Vector.Red, Material.Diffuse.Vector.Green, Material.Diffuse.Vector.Blue, Material.Diffuse.Vector.Alpha);
...
function T3DModel.Load(Data: TStream): Boolean;
begin
  Clear;
  FRootChunk:=TChunk.Create(Self);
  FFile:=Data;
  FRootChunk.FFile:=FFile;
  FRootChunk.Read;
