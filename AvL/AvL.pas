...
//   property MainMenu: TMenu read FMenu write SetMainMenu;
   ...
   property WindowState: Integer read FWindowState write SetWindowState;
...
const
  DefFont : array[1..14] of Char = ('M', 'S', ' ', 'S', 'a', 'n', 's', ' ', 'S', 'e', 'r', 'i', 'f', #0);
asm
  mov [EAX].FName, offset[DefFont]
  mov [EAX].FColor, clBlack
  mov [EAX].FHeight, -11
  mov [EAX].FCharset, 1
//  mov [EAX].FStyle:= [];
//  mov [EAX].FSize := 8;
...
asm
  mov [EAX].FParent, AParent
  mov [EAX].FEnabled, True;
  mov [EAX].FShowHint, True;
  mov [EAX].FCtl3D, True;
  mov [EAX].FLeft, 2;
  mov [EAX].FTop, 2;
end;
...
// inherited Create;
  ...
  FShowHint := True;
  FCtl3D := True;
  FLeft := 2;   //Это сделано чтоб контролы не появлялись
  FTop := 2;    //при создании в самом углу формы
end;
{$endif}
...
begin
  FWindowState := Value;
  ShowWindow(FHandle, FWindowState);
end;
...
//  if Handle <> 0 then
  ...
  if result then FileName:=copy(tempFileName, 1, pos(#0, TempFilename)-1);
end;
  ...
  while i<=High(Template) do
   begin
    ...
    if Template[i+1]='(' then
     ...
     if Template[i]<>')' then AppendMenu(FHandle, mnuType, i+Adder, Template[i]);
    Inc(i)
   end;
...
constructor TRichEdit.Create(AParent: TWinControl; Text: String);
const
...
begin
  if Assigned(FOnKeyDown) then FOnKeyDown(Self, AMsg.CharCode, []);
  Dispatch(AMsg);
end;

procedure TWinControl.WMKeyUp(var AMsg: TWMKeyUp);
begin
  if Assigned(FOnKeyUp) then FOnKeyUp(Self, AMsg.CharCode, []);
  Dispatch(AMsg);
end;
...
//  FExStyle := WS_EX_CLIENTEDGE ;
  ...
  if Flat then FStyle := FStyle or TBSTYLE_FLAT;
  //FColor := clBtnFace ;
  CreateWnd;
